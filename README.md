...
This package offers an easy way to instantiate a class, to send typed input to it and to receive typed result using a convenient interface:
```php
public function useUtility(string $abstract, array $input): mixed
```
It is implemented by `Resolver` class. It takes care to there are no duplicated instances and resets input/output before using.
<br>
...
```
Тhe data sent to `useUtility` through `array $input` is available as class properties in the instance. The output, that we need, should be set in `output` property. In this way we may rely on typed input and output without binding the executed code to a certain interface.
...
- In `defaultInput` add the properties names with theirs default values. When `useUtility` is called these values are available unless they aren't sent to it through the second parameter.
