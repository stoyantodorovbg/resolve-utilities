...
test('Utility validates output to be bool and returns it', function () {
    $value = true;
    $output = $this->utilityHelper->getUtility(SuccessBoolOutput::class, $value);
    $this->assertSame($value, $output);
});

test('Utility validates output to be string and returns it', function () {
    $value = 'value';
    $output = $this->utilityHelper->getUtility(SuccessStringOutput::class, $value);
    $this->assertSame($value, $output);
});

test('Utility validates output to be integer and returns it', function () {
    $value = 1;
    $output = $this->utilityHelper->getUtility(SuccessIntegerOutput::class, $value);
    $this->assertSame($value, $output);
});

test('Utility validates output to be float and returns it', function () {
    $value = 1.1;
    $output = $this->utilityHelper->getUtility(SuccessFloatOutput::class, $value);
    $this->assertSame($value, $output);
});

test('Utility validates output to be array and returns it', function () {
    $value = [];
    $output = $this->utilityHelper->getUtility(SuccessArrayOutput::class, $value);
    $this->assertSame($value, $output);
});

test('Utility validates output to be object and returns it', function () {
    $value = new stdClass;
    $output = $this->utilityHelper->getUtility(SuccessObjectOutput::class, $value);
    $this->assertSame($value, $output);
});
...
test('Utility validates output to be class instance and returns it', function () {
    $value = new TestClass;
    $output = $this->utilityHelper->getUtility(SuccessClassInstanceOutput::class, $value);
    $this->assertSame($value, $output);
});
...
test('Utility validates output to be int or float and returns it', function () {
    ...
    $value = 1;
    $output = $this->utilityHelper->useUtility($utility, $value);
    $this->assertSame($value, $output);
